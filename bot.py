...
def make_request(endpoint, data=None, useragent=None):
    headers = {
...
def daily_checkin(init_data, useragent):
...
def claim_ads(init_data, useragent):
    user_id = parse_user_id(init_data)
    ...
    while True:
        ...
        response = make_request('watched.php', payload, useragent=ua)
...
if __name__ == "__main__":
    ...
    for init_data in users:
        ...
        daily_checkin(init_data, ua)

        # Step 2: Claim ads until hourly limit reached
        claim_ads(init_data, ua)
